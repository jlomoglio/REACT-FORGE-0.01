...
export default function ConfirmSoftDeleteNoteModal({ isOpen, onCancel, onConfirm, noteId, noteName }) {
                    ...
                    <div className="flex min-h-full items-center justify-center p-4">
                        <TransitionChild as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" leave="ease-in duration-200" leaveTo="opacity-0 scale-95" enterTo="opacity-100 scale-100">
                            <DialogPanel className="w-full max-w-md transform rounded bg-[#1e1e1e] p-6 text-white shadow-xl border border-gray-600 transition-all">
                                <DialogTitle className="text-lg font-bold mb-4">Move Note to Trash</DialogTitle>
                                <p className="text-sm text-gray-300 mb-6">
                                    Are you sure you want to move note <strong>{noteName}</strong> to the trash? 
                                </p>
                                <div className="flex justify-end gap-4">
                                    <button onClick={() => onConfirm(noteId)} className="px-4 py-2 bg-red-600 hover:bg-red-500 rounded">
                                        Ok
                                    </button>
